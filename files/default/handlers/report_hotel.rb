  ...
  class WarHotelHandler < Chef::Handler
    def report
      ...
      if run_status.updated_resources.empty?
        ...
        message << "Subject: WAR Hotel configuration report for #{node['war_hotel']['id']}\n"
        message << "Date: #{Time.now.rfc2822}\n"
        message << "\n"

        message << "Greetings,\n"
        message << "\n"
        message << "Chef just recently configured the WAR Hotel with the ID #{node['war_hotel']['id']}.\n"
        message << "\n"
        message << "\n"
        ...
        message << "Linux Distro: #{node['platform']} #{node['platform_version']}\n"
        message << "Java Version: #{node['languages']['java']['version']}\n"
        message << "Maven Version: #{node['war_hotel']['maven_version']}\n"
        message << "Docker Version: #{node['war_hotel']['docker_version']}\n"
        message << "\n"
      ...
      Net::SMTP.start('localhost', 25) do |smtp|
        smtp.send_message message, 'btwebb@gmail.com', 'btwebb@gmail.com'
      end
